...
public class Move : MonoBehaviour {
	...
	void Start () {
		GetComponent<Rigidbody>().velocity = Vector3.down * minVelocity;
	}
	...
	void FixedUpdate () {
		if (GetComponent<Rigidbody>().velocity.magnitude < minVelocity/2) {
		...
		if (velocity < maxVelocity && GetComponent<Rigidbody>().velocity.magnitude > velocity - 5) {
		...
		this.GetComponent<Rigidbody>().velocity = dir * velocity;
	}
